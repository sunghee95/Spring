...
public class TransactionController {
	 ...
	 * 3. 트랜잭션 적용
	 * 	
	 * 	[테스트는 AOP가 적용된 CrudBoardController와 연계된 ServiceImpl 에서 진행]
	 * 
	 ...
	 * 		삭제, 수정도 마찬가지로 이루어짐
	 * 
	 * 		@Transactional을 적용하고 중간에 에러를 발생시킨 후, 롤백된 상태를 확인하려고 했으나 롤백이 되지 않고 데이터베이스에 데이터가 등록된다 
	 * 		
	 * 		이유는 스프링 프레임워크에서 @Transactional 어노테이션은 기본적으로 Checked Exception에 대해서는 롤백 처리를 하지 않도록 설계되어 있다 
	 * 		기본적으로 스프링에서 트랜잭션 처리는 RuntimeException 계열이라면 Rollback 처리한다 
	 * 		그러나 UncheckedException에 대해서는 Rollback 처리 진행하지 않는다
	 * 
	 * 		여기서, 트랜잭션으로 국한된 롤백 정책이 아니라 스프링 프레임워크에서의 기본 정책에 대한 내용일 뿐이다 
	 * 		실제로 '트랜잭션 롤백 처리', 'Checked Exception vs Unchecked Exception' 등 검색 해보면 잘못 된 정보를 복붙 해놓은 내용
	 * 		수두룩하며 많은 에러 정보를 공유(모든 블로그가 그렇진않지만 대표적인 표가 무수히 많이 공유되고있음)
	 * 
	 * 		*** 
	 * 		'예외처리 시 트래잭션 처리'에 대한 정보가 잘못되어 표기된 경우가 많다
	 ...
	 *  	- Error : 시스템에서 발생하는 심각한 수준의 에러로 개발자가 대응할 수 없기 때문에 예외 처리에 대한 부분을 신경쓰지 않아도 된다 
	 *  	
	 *  	CheckedException vs Unchecked Exception
	 *  	- RuntimeException의 상속 여부에 따라서 CheckedException과 Unchecked Exception로 나뉘어진다 
	 *  	
	 *  					| 	Checked Exception				| 			Unchecked Exception
	 *  _________________________________________________________________________________________________________________
	 *  	예외처리 여부		| 	반드시 예외 처리 코드가 있어야함 			| 		강제로 예외 처리는 아님 
	 *  	예외확인 시점		| 	컴파일 단계에서부터 컴파일이 되지 않음 	| 		런타임중 예외가 확인됨  
	 *  	클래스 			| 	 IOException, SQLException.. 	| 		NullPointException, IndexOutOfBoundException...
	 *  
	 *  	**** 트랜잭션도 AOP의 개념이 반영된 관점 지향 프로그래밍이라 할 수 있음 
	 *  	
	 ...
	 *  		- 새롭게 시작된 트랜잭션에서만 적용되므로, Propagation.REQUIRED또는 Propagation.REQUIRES_NEW와 함께 사용되도록 독점 설계되었다
	 *  		
	 *  		1-1) 옵션 
	 *  		
	 *  		** 용어
	 ...
	 *  		
	 *  		#Phantom Read
	 *  		- 자신이 처리중인 트랜잭션에서 처리했던 내용 안에서 다른 트랜잭션이 데이터를 수정 후 Commit하더라도 자신의 트랜잭션에서 처리한 내용만 사용하는 것을 의미 
	 *  
	 *   		[Option]
	 ...
	 *  		- REPEATABLE_READ : Dirty Read, Non-repeatable Read가 방지됨을 나타내는 상수이다 
	 *  							Phantom Read가 발생할 수 있다 
	 *  			> 트랜잭션 Commit이 일어난 데이터에 대해서 조회 가능(트랜잭션 완료시까지 조회에 대한 Shared lock이 걸리지 않음) 
	 *  		- SERIALIZABLE : Dirty Read, Non-repeatable Read 및 Phantom Read가 방지됨을 나타내는 상소이다 
	 *  			> Phantom Read 가 발생하지 않는다 
	 *  			> 거의 사용하지 않는 옵션 *****
	 *  
	 *  		2) propagation(전파옵션)
	 *  		- 기존 진행중인 트랜잭션 외에 추가적으로 진행중인 트랜잭션이 존재할 때 추가적인 트랜잭션에 대해서 어떻게 처리할지에 대한 설정 
	 *  		- 추가적인 트랜잭션을 기존 트랜잭션에 포함시켜 함께 처리할 수도 있고, 추가적인 트랜잭션처럼 별도의 트랜잭션으로 추가 할수도 있고 
	 ...
	 *  				> 부모 / 자식간에 상관관계에서 자식부분의 트랜잭션이 rollback 처리시 , 부모까지 영향이 가서 rollback 처리된다 
	 *				- REQUIR ES_NEW : 새로운 트랜잭션을 생성한다 
	 *					> rollback은 각각 이루어진다 
	 *				- SUPPORT :  트랜잭션이 있으면 현재 트랜잭션을 지원하고 트랜잭션이 없으면 트랜잭션이 아닌 방식으로 실행한다 
	 ...
	 *					> 읽기 전용에 대한 힌트를 분석할 수 있는 트랜잭션인 경우 throw를 던지지 않고 조용히 힌트를 무시 
	 *
	 *			4) timeout(트랜잭션 시간제한)
	 *				- 기본값은 -1로 무제한
	 ...
	 *			
	 *			5) rollbackFor
	 *				- 트랜잭션 롤백을 유발해야 하는 예외 유형을 나타내는 0개 이상의 예외유형을 정의
	 *				- 기본적으로 트랜잭션은 롤백되지만, RuntimeException계열과 Error는 롤백되지 않음 
	 *				- 기본적인 정책 이외에 에러를 처리할 경우, 해당 에러를 선언하여 롤백 정책을 추가  
	 */
