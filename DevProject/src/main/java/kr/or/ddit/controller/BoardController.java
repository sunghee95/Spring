...
public class BoardController {
	 ...
	 *  - 속성이 하나일 때는 속성명이 생략될 수 있음
	 *  - 컨트롤러의 클래스 레벨과 메서드 레벨로 지정할 수 있음
	 *  - 클래스 레벨로 요청 경로를 지정하면 메소드 레벨에서 지정한 경로의 기본 경로로 취급함
	 ...
	 * 
	 * - 해당 경로로 요청시, board 폴더에 read.jsp를 찾아서 페이지를 출력해주는 것 같지만,
	 *   경로상으로만 보면 board/read/boardNo라는 파일까지 찾게된다
	 * 
	 * - URL 경로상의 변하는 값을 경로 변수로 취급
	 * - 경로 변수에 해당하는 값을 파라미터 변수에 설정할 수 있다
	 */
	
	@RequestMapping(value="/read/{boardNo}")      // 여기의 boardNo을 받는 것 
	public String read(@PathVariable("boardNo") int boardNo) {  // 브라우저에서 보낸 파라미터를 꺼내는게  아니라 
		logger.info("read()실행 !!!!");
...
//	}
	...
	// Header 매핑이지만, headers 설정을 하지 않고 요청
	...
	/*
	 * 6. Content Type 매핑 
	 * - 요청의 ContentType 헤더 값을 매핑 조건으로 지정하는 경우에는 consumes 속성을 사용
	 */
	// 응답 상태 코드와 값을 제네릭에 설정되어 있는 타입에 따라서 내 마음대로 내보낼 수 있다
	// 응답상태 코드 : HTTPStatus라는 객체 안에 있는 여러 상태 코드를 선택해서 내보낼 수 있다
	// 결과 : 제네릭 타입에 설정되어있는 해당 타입의 값으로 결과의 값을 설정해서 내보낼 수 있다
	
	// consumes 속성값을 지정하지 않고 테스트
	// consumes 속성값을 지정하지 않으면 기본값으로 "application/json" 미디어 타입으로 지정 
	...
	// consumes 속성값에 "application/json" 미디어 타입을 지정 
	...
	public ResponseEntity<String> modifyContentTypeByXml(
	  ...
	  * 
	  * produces 속성값을 지정하지 않고 테스트       비동기처리할 때 많이 쓰임 !!!!!!!!!!!!!!!!!! 
	  * produces 속성값을 지정하지 않으면 기본값인 "application/json" 미디어 타입으로 지정됨
	  */
	...
	// produces 속성 값에 "application/json" 미디어 타입 지정 
	@RequestMapping(value="/{boardNo}", method = RequestMethod.GET, produces = "application/json") // ajax에서 headers로 보낸 부분과 매핑
	...
	// produces 속성 값에 "application/xml" 미디어 타입 지정 
	@RequestMapping(value="/{boardNo}", method = RequestMethod.GET, produces = "application/xml") // ajax에서 headers로 보낸 부분과 매핑
