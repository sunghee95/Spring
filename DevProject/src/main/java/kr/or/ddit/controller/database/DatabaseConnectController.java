...
public class DatabaseConnectController {
	 ...
	 *  		
	 *  	1. JDBCTemplate클래스가 제공하는 주요 메서드
	 *  	
	 *  	queryForObject
	 *  	- 하나의 결과 레코드중에서 하나의 컬럼 값을 가져온다 
	 *  	queryForMap
	 ...
	 *  	3. 게시판 구현 
	 *  	
	 *  	기본적인 JDBC를 이용한 DB설계는 DAO란에서 sql 쿼리가 노출되어 개발하는 방식이다보니 우리가 중급프로젝트 또는 JSP에서 
	 ...
	 * 			@Table(name="board")
	 * 			public class Board{...
	 * 		그리고 각 필드 (멤버 변수)에 @Column(name="board_no") 어노테이션으로 테이블 컬럼과 매핑 설정을 한다 
	 * 		설정된 Entity들을 이용하여 JPA 객체를 통해서 함수를 호출하면 (persist, find등등 ) 호출된 함수를 통해 Spring Data JPA가
	 ...
	 * 		발생하지만 JPA는 필드만 추가후 테이블 정보와 매핑만 시켜주면 JAP가 자동으로 분석을 진행한다 
	 * 		그리고, 사용하는 데이터베이스 종류가 변경된다 하더라도 문제가 없다 ( JPA는 SQL을 직접 건드리지 않고 함수와 같은 기능들로만 쿼리를 요청하고 
	 * 		데이터베이스와 통신하기 때문이다)
	 * 
	 *   	JPA는 JPA 구현체인 hibernate를 이용하여 사용한다. 그러기 위해선 관련 의존 관계를 등록 후 사용해야한다 
	 *   	SQL문을 이용하여 데이터베이스를 연동하여 데이터를 가공하느 ㄴ게 아닌, Method를 통해 데이터베이스를 조작할 수 있다는 장점이 있어 객체모델을 
	 *   	이용하여 비즈니스 로직을 구성하는데에만 집중을 할 수 있다. 하지만, 프로젝트의 규모와 복잡도가 클 때는 설계가 잘못되는 경우 해당 프로젝트의
	 *   	품질이 떨어진다는 문제가 있따. 그만큼 설계라인의 정확성이 필요하고 여러 방면에서 정확한 검증이 제대로 끝나지 않으면 적용하기가 까다롭다 
	 *   
	 *   
	 *   --------------------------------------------
	 *   [용어]
	 ...
	 *   	- 저는 ibatis를 사용했습니다, mybatis 사용했어여 와 같은 대답을 하면된다 
	 * 
	 * 7. 마이바티스 
	 * - SQL과 자바 객체를 매핑하는 아이디어로 개발된 데이터베이스 접근용 프레임 워그 
	 * 	마이바티스 관련내용은 12장에서 더 자세하고 알고 넘어간다 
	 * 
	 *   	1) 의존 관계 정의 
	 ...
	 *    	- 예제로 mapper패키지안에 BoardMapper.xml을 구성하여 테스를 진행 
	 * 		
	 * 
	 */
