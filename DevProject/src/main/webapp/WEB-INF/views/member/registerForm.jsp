...
<body>
	...
	<p>1) URL 경로상의 쿼리 파라미터 정보로부터 요청 데이터를 취득할 수 있다</p>
	<a href="register?userId=hongkd&password=1234">BUTTON1</a>
	<br/>
	
	<p>2) URL 경로상의 경로 변수로부터 요청 데이터를 취득할 수 있다
	...
	<p>5) HTML Form 필드가 여러개일 경우에 컨트롤러 매개변수의 위치는 상관 있는가 ?</p>		
	...
	<p>1) URL 경로상의 경로 변수가 여러개일때 @PathVariables 어노테이션을 사용하여 특정한 경로 변수명을 지정해준다</p>
	<a href="/register/hongkd/100">BUTTON3</a><br/>
	
	
	<p>2) HTML Form의 필드명과 컨트롤러 매개변수명이 일치하면 요청을 처리할 수 있다</p>
	...
	<p>3) HTML Form 필드명과 컨트롤러 매개변수명이 일치(대소문자 구분)하지않으면 요청을 처리할 수 없다</p>		
	...
	<p>4) @RequestParam  어노테이션을 사용하여 HTML Form의 필드명을 지정하여 요청을 처리한다</p>	
	...
	<p> 2)  쿼리 파라미터 ( dateOfBirth=2018-09-08)로 전달받은 값이 날짜 문자열 형식으로 설정시, Date 타입으로 받는가 ? </p>
	<a href="/registerByGet01?userId=hongkd&dateOfBirth=2018-09-08">BUTTON5</a>
	
	<p>3) 쿼리 파라미터(dateOfBirth=20180908)로 전달받은 값이 날짜 문자열 형식으로 설정시, Date 타입으로 받는가 ?</p>
	<a href="/registerByGet01?userId=hongkd&dateOfBirth=20180908">BUTTON6</a>
	
	<p>
		4) 쿼리 파라미터 (dateOfBirth=2018/09/08)로 전달받은 값이 날짜 문자열 형식으로 설정 시, Date 타입으로 받는가 ?
	...
	<p>4) 폼 라디오버튼  요소값을 기본 데이터 타입인 문자열 매개변수로 처리한다</p>
	...
	<p>6) 복수 선택이 가능한 폼 셀렉트 박스 요소값을 기본 데이터 타입인 문자열 매개변수로 처리한다</p>
		<form action="/registerMultiSelect01" method="post">
		 	...
		 	<input type="submit" value="registerMultiSelect01"/>
		</form><br/>	
	...
	<p>7) 복수 선택이 가능한 폼 셀렉트 박스 요소값을  문자열 배열  타입 매개변수로 처리한다</p>
		<form action="/registerMultiSelect02" method="post">
		 	...
		 	<input type="submit" value="registerMultiSelect02"/>
		</form><br/>	
	...
	<p>8) 복수 선택이 가능한 폼 셀렉트 박스 요소값을 문자열 요소를 가진 리스트 컬렉션 타입 매개변수로 처리한다</p>	
		<form action="/registerMultiSelect03" method="post">
		 	...
		 	<input type="submit" value="registerMultiSelect03"/>
		</form><br/>	
	...
	<form action="/registerAddress" method="post">
		...
		<input type="submit" name="registerAddress"/><br/> 
	</form><br/> 
	...
	<form action="/registerUserAddress" method="post">
		...
		<input type="submit" name="registerUserAddress"/><br/> 
	</form><br/> 
	...
	<form action="/registerUserCardList" method="post">
		...
		<input type="submit" name="registerUserCardList"/><br/> 
	</form><br/> 
	...
	<p>18) 폼 텍스트 영역 요소값을 Date타입 매개변수로 처리한다</p>
